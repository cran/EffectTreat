plot.PCA.ContCont <- function(x, Xlab.PCA, Main.PCA, Type="Percent", Labels=FALSE, PCA=TRUE, 
  ...
  if (Good.Pretreat==TRUE){
    ...
    if (Type=="Freq"){
      ...
      if (Labels==TRUE){
        plot(h,freq=T, xlab=expression(delta), ylab="Frequency", col=col, labels=labs, main=Main.Good.Pretreat)
      }
    }
    ...
    if (Type=="Percent"){
      ...
      if (Labels==TRUE){
        plot(h,freq=F, xlab=expression(delta), ylab="Percentage", col=col, labels=labs, main=Main.Good.Pretreat)
      }
    }
    ...
    if (Type=="CumPerc"){
      ...
      h$density <- h$counts/sum(h$counts)
      cumulative <- cumsum(h$density)
      plot(x=h$mids, y=cumulative, xlab=expression(delta), ylab="Cumulative percentage", col=0, main=Main.Good.Pretreat)
      lines(x=h$mids, y=cumulative)
    }
